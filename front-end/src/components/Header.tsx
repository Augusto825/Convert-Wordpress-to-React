...
const Header: React.FC<HeaderProps> = ({ navItems }) => {
  ...
  return (
    <header
      className="fixed top-0 left-0 w-full z-50 transition-colors duration-300 content-center"
      style={{ backgroundColor: bgColor, minHeight: '100px' }} // Changed from 90px to 100px
    >
      <div className="container mx-auto flex items-center justify-between px-4 md:px-96 h-full">
        {/* Navigation and Logo Wrapper */}
        <div className="flex items-center justify-between w-full md:w-[1140px]">
          {/* Desktop Navigation */}
          ...
          {/* Logo */}
          <div className="flex-shrink-0 ml-5 w-[120px] h-[80px]"> {/* Adjusted logo height */}
            <img
              loading="lazy"
              src={Logo}
        ...
        {isOpen && (
          <div className="md:hidden absolute top-full left-0 w-full bg-[#4E0559]">
            <ul className="flex flex-col items-center py-2 space-y-2">
              {navItems.map((item, index) => (
                <NavItem key={index} text={item} onClick={() => setIsOpen(false)} />
              ))}
            </ul>
          </div>
        )}
